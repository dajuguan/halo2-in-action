...
use halo2_proofs:: {
    plonk::*, circuit::*, pasta::group::ff::PrimeField, poly::Rotation, 
};
...
impl <F:PrimeField, const NUM_BITS: usize, const RANGE: usize> RangeCheckConfig<F, NUM_BITS, RANGE> {
    ...
    fn assign(
        ...
        layouter.assign_region(|| "bit&value region", |mut region|{
            for i in 0..bits.len() {
...
impl <F:PrimeField, const NUM_BITS: usize, const RANGE: usize> Circuit<F> for MyCircuit<F,NUM_BITS,RANGE> {
    ...
    fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<F>) -> Result<(), Error> {
        ...
        let bits = self.num_bits.iter().map(|v| Value::known(F::from(*v as u64))).collect();
...
mod tests {
    ...
    fn plot_lookup2_circuit(){
        ...
        let root = BitMapBackend::new("./images/lookup2.png", (1024, 768)).into_drawing_area();
        root.fill(&WHITE).unwrap();
        let root = root
            .titled("Lookup2 Circuit", ("sans-serif", 60))
            .unwrap();
